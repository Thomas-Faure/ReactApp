...
class BackOfficeShowReportComments extends Component {
    ...
    search(){
      var temp = this.state.dataFixed.filter((n)=>{
        var properties = ["comment","nbReport","description"]
        var exist = false
    ...
    validate(id){
        const token = localStorage.token;
          fetch('http://51.255.175.118:2000/comment/' + id+'/validate', {
            method: 'PUT',
            headers: {
              'Authorization': 'Bearer ' + token
            }
  ...
  <div className="modal-card">
    ...
    {this.state.action =="validate" ?
      ...
      <button className="button" onClick={()=>{this.setState({isOpen:false})}}>Cancel</button>
            ...
            <h1 style={{textAlign: "center",fontWeight: "bold",fontSize: "30px",marginBottom:"10px"}}>Manage Comments Report</h1>
...
<p style={{textAlign: "center",margin: "auto"}}><span style={{marginBottom:"10px"}}>The actual page is : {this.state.actualPage} / {this.state.maxPage}</span><br/>
